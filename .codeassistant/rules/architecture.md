...
- **cypress** - End-To-End тесты пользовательского интерфейса.
- **docker** - Файлы для сборки Docker контейнеров.
- **frontend** - Реализация пользовательского интерфейса (Vue.js).
- **mock-server** - Конфигурация для [HTTP mock server](https://github.com/jmartin82/mmock)

## Правила зависимостей между слоями

**Каждый слой может использовать только нижестоящие слои:**

| Слой | Может использовать |
|------|-------------------|
| **Presentation** | Infrastructure, Application, Domain (своего модуля и чужих) |
| **Infrastructure** | Application, Domain (своего и чужих модулей) |
| **Application** | Domain (предпочтительно только своего модуля) |
...
Модули взаимодействуют двумя способами:
